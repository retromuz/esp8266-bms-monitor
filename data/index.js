class Cell {
	...
	constructor(width, height, zoom) {
		...
		this.inline.attr({fill: '#ffffff', 'fill-opacity': 1.0});
		this.rect = this.draw.rect({width: 0, height: this.RECT_HEIGHT});
		this.rect.move(this.RECT_LOC_X, this.RECT_LOC_Y);
		this.rect.attr({fill: 'green', 'fill-opacity': 1.0});
		this.rect.radius(4);
		this.text = this.draw.text('---mV');
		this.text.move(14 * this.zoom,(this.height - 10) * this.zoom).font({ fill: '#000000', size: 76, family: 'monospace' })
		this.draw.viewbox({ x: 0, y: 0, width: this.width * this.zoom, height: this.height * this.zoom})
	}
	...
	setStateContinuous(width) {
		...
		r = r.length == 1 ? '0' + r : r;
		g = g.length == 1 ? '0' + g : g;
		this.rect.attr({fill: '#' + r + g + '00', 'fill-opacity': 1.0});
	...
	setVoltage(voltage) {
		if (voltage > 0 && voltage < 5000) {
			let range = 4200 - 3200;
			let rel = voltage - 3200;
			rel = rel < 0 ? 0 : rel;
			rel = rel > range ? range : rel;
			this.setStateContinuous(rel*267/range);
			this.text.tspan(voltage + 'mV');
		}
	}
   ...
   $.ajax({
        ...
        success: function (data) {
            ...
            for (let x = 0; x < data.length; x++) {
                cells[x].setVoltage(data[x]);
            }
            setTimeout(function(){b(cells);}, 1200);
        },
...
function b(cells) {
	$.ajax({
		...
		success: function (data) {
			...
			setTimeout(function(){v(cells)}, 1200);
		},
		error: function(xhr, opts, err) {
			setTimeout(function(){v(cells);}, 1200);
		}
